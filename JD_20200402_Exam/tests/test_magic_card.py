...
class TestMagicCard(TestCase):
    ...
    def setUp(self):
        self.test_card = MagicCard("test_card")

    def test_initialization(self):
        self.assertEqual("test_card", self.test_card.name)
        self.assertEqual(80, self.test_card.health_points)
        self.assertEqual(5, self.test_card.damage_points)

    def test_invalid_name(self):
        invalid_name = ""
        with self.assertRaises(ValueError) as context:
            MagicCard(invalid_name)
